...
public class CreateContestCommand implements ICommand{
    ...
    public void execute(List<String> tokens) {
       /*  if(tokens.size() == 5){
            String contestName = tokens.get(0);
            String levelstr = tokens.get(1);
            String contestCreator = tokens.get(2);
            int numQuestion = Integer.parseInt(tokens.get(3));
        ...
        {       
            ...
            try{
                System.out.println(contestService.create(contestName, Level.valueOf(level), contestCreator, numQuestion).toString());
            }
        ...
        else
        {
            String contestName = tokens.get(1),level = tokens.get(2),contestCreator = tokens.get(3);
            try{
                System.out.println(contestService.create(contestName, Level.valueOf(level), contestCreator,null).toString());
            }
            catch(UserNotFoundException uxe)
            {
                System.out.println("Contest Creator for given name: creator not found!");

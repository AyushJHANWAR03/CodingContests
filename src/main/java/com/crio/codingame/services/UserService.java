...
public class UserService implements IUserService {
    ...
    public List<User> getAllUserScoreOrderWise(ScoreOrder scoreOrder){
        ...
        if(scoreOrder == ScoreOrder.ASC){
            user.sort(Comparator.comparing(User::getScore));
        }
        else{
            user.sort(Comparator.comparing(User::getScore).reversed());
        }
        return user;
    }
    ...
    public UserRegistrationDto withdrawContest(String contestId, String userName) throws ContestNotFoundException, UserNotFoundException, InvalidOperationException {   
        Contest contest = contestRepository.findById(contestId).orElseThrow(() -> new ContestNotFoundException("Contest are unavailable"));
        User user = userRepository.findByName(userName).orElseThrow(() -> new UserNotFoundException("User is not found"));
        if(contest.getContestStatus().equals(ContestStatus.IN_PROGRESS)){
            throw new InvalidOperationException(contestId);
        }
        if(contest.getContestStatus().equals(ContestStatus.ENDED)){
            throw new InvalidOperationException(contestId);
        }
